const baseUrl = process.env.SMOKE_BASE_URL || 'http://127.0.0.1:5240';
...
const adminCookie = await login('admin@fitness-studio-franchise.local', 'admin123');
const managerCookie = await login('manager@fitness-studio-franchise.local', 'manager123');
const analystCookie = await login('analyst@fitness-studio-franchise.local', 'analyst123');
